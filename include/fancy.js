...
// todo oliver steele's library also defines underscore. I should use a different technique here...
    ...
    AlexLibrary.filterObj = AlexLibrary.selectObj = function (obj, callback) {
        return obj.pairs().filter(function (a) {
            return callback(a[1], a[0]);
        }).object();
    };

    AlexLibrary.rejectObj = function (obj, callback) {
        return obj.pairs().reject(function (a) {
            return callback(a[1], a[0]);
        }).object();
    };

    AlexLibrary.mapObj = function (obj, callback) {
        return  obj.pairs().map(function (pair) {
            return [pair[0], callback(pair[1], pair[0])];
        }).object();
    };
    ...
    AlexLibrary.difference = function (arrayEsque) {
        ...
        return _.filter(arrayEsque, function (val, key) {
            return !toRemove.hasOwnProperty(val);
        });
    }
    ...
    var ref = function (reverse) {
        return function (obj, toRemove) {
            ...
            var copy = obj.rejectObj(function (val, key) {
                return reverse ^ objToRemove.hasOwnProperty(key);
            });
            return copy;
        };
    }
    ...
    AlexLibrary.meld = function (me, targetObjs, reducer, filteringUnsets) {
        ...
        allObjects = allObjects.map(function (obj) {
            return FO(obj);
        });
        allObjects.push(me);
        ...
        allKeys.map(function (key) {
            ...
            if (filteringUnsets === true)
                resultArray = resultArray.filter(function (a) {
                    return (typeof a !== "undefined");
                });
            else if (!filteringUnsets)
                resultArray = resultArray.map(function (a) {
...
//this is begging to be rewritten as a right apply on divide composed with parseInt passed to _.groupBy
    ...
    FancyArray = FA = function (/*Array*/ input) {
        if (!input)
            input = [];
        if (!(this instanceof FancyArray)) {  //if they forgot to use new, then pretend they used new
        ...
        if (compatibility_mode) {
            ...
            if (typeof result.__proto__ == "object") {
                if (!compatibility_mode_for_ie) { // FUture note: there used to be a browser called IE
                    return this;
                }
    ...
    FancyArray.range = function () {
        return FancyArray(_.range.apply(_, arguments));
    };
    FancyArray.make = function (i) {
        return new FancyArray(i);
    };
    FancyArray.makeArray = function (i) {
    ...
    var polyfillColl = [
        {"name": "each", iterator: 1},
        {"name": "map", iterator: 1},
        {"name": "reduce", iterator: 1},
        {"name": "reduceRight", iterator: 1},
        {"name": "find", iterator: 1},
        {"name": "where"},
        {"name": "findWhere"},
        {"name": "reject", iterator: 1},
        {"name": "every", iterator: 1},
        {"name": "some", iterator: 1},
        {"name": "contains"},
        {"name": "invoke"},
        {"name": "pluck"},
        {"name": "max", iterator: 1},
        {"name": "min", iterator: 1},
        {"name": "sortBy", iterator: 1},
        {"name": "groupBy", iterator: 1},
        {"name": "countBy", iterator: 1},
        {"name": "shuffle"},
        {"name": "toArray"},
        {"name": "size"}
    ];
    ...
    var polyfillArr = [
        ...
        {"name": "union"},
        {"name": "intersection"},
        {"name": "uniq", iterator: 2},
        {"name": "zip"},
        {"name": "object"},
        {"name": "indexOf"},
        {"name": "lastIndexOf"},
        {"name": "sortedIndex", iterator: 2},
        {"name": "range"},
        {"name": "filter", iterator: 1, assumeIdentity: true}
    ];
    ...
    var alexLibObj = [
        {"name": "pick"},
        {"name": "omit"},
        {"name": "mapObj", iterator: 1},
        {"name": "filterObj", iterator: 1},
        {name: "selectObj", iterator: 1},
        {name: "rejectObj", iterator: 1},
        {name: "meld", iterator: 2}
    ];
    var alexLibArr = [
        {name: "toTrueArray"},
        {name: 'apply', iterator: 1},
        {name: "slice"},
        {name: "sameContents"},
        {name: "hasAll"},
        {name: "chunk"},
